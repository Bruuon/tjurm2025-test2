...
std::vector<cv::Mat> erode(const cv::Mat &src_erode, const cv::Mat &src_dilate)
{
  ...
  // TODO: 在这里实现你的代码
  const int thresh = 50, maxval = 255;
  const cv::Point anchor(-1, -1);
  const auto kernel = cv::getStructuringElement(cv::MorphShapes::MORPH_RECT, cv::Size(8, 8), anchor);

  cv::cvtColor(src_erode, dst_erode, cv::COLOR_BGR2GRAY);
  cv::threshold(dst_erode, dst_erode, thresh, maxval, cv::THRESH_BINARY);
  cv::erode(dst_erode, dst_erode, kernel, anchor, 1, cv::BORDER_CONSTANT, cv::morphologyDefaultBorderValue());

  cv::cvtColor(src_dilate, dst_dilate, cv::COLOR_BGR2GRAY);
  cv::threshold(dst_dilate, dst_dilate, thresh, maxval, cv::THRESH_BINARY);
  cv::dilate(dst_dilate, dst_dilate, kernel, anchor, 1, cv::BORDER_CONSTANT, cv::morphologyDefaultBorderValue());
